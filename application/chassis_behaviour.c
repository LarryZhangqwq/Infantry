...
static void chassis_infantry_follow_gimbal_yaw_control(fp32 *vx_set, fp32 *vy_set, fp32 *angle_set, chassis_move_t *chassis_move_rc_to_vector)
{
  ...
  if (chassis_move_rc_to_vector->chassis_RC->rc.ch[4] < -100||(chassis_move_rc_to_vector->chassis_RC->key.v & SWING_KEY))
  {
    chassis_move_rc_to_vector->swing_flag = 1;
  }
	else if (chassis_move_rc_to_vector->chassis_RC->rc.ch[4] > 100)
  {
    chassis_move_rc_to_vector->swing_flag = 2;
  }
}
