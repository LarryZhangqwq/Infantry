...
/**
  ...
  * @retval         none
  */
fp32 supercap_vx_set = 0.4;
fp32 supercap_vy_set = 0.4;
fp32 normal_vx_scale = 275.0;
fp32 normal_vy_scale = 275.0;
...
static void chassis_set_contorl(chassis_move_t *chassis_move_control)
{
  ...
  if (chassis_move_control->chassis_mode == CHASSIS_VECTOR_FOLLOW_GIMBAL_YAW)
  {
    if (chassis_move_control->swing_flag == 1)
    {
      ...
      //set control relative angle  set-point
			if(cap_state)
			{
				...
				chassis_move_control->wz_set = 2;//chassis_power / wz_scale_cap;
			}
			else
			{
				chassis_move_control->wz_set = 1;//chassis_power / wz_scale;
			}
      //speed limit
      //速度限幅
      ...
      chassis_move_control->vy_set = fp32_constrain(chassis_move_control->vy_set, chassis_move_control->vy_min_speed, chassis_move_control->vy_max_speed);
		...
		else if (chassis_move_control->swing_flag == 2)
		{
			top_sin += 0.001;
			 fp32 sin_yaw = 0.0f, cos_yaw = 0.0f;
      //rotate chassis direction, make sure vertial direction follow gimbal
      ...
      //set control relative angle  set-point
			...
			{
                //这里的chassis_power是最大功率
				chassis_move_control->wz_set = 2 * fabs(arm_sin_f32(top_sin)) + 1;//chassis_power / wz_scale_cap;
			}
			else
			{
				chassis_move_control->wz_set = 1 * fabs(arm_sin_f32(top_sin)) + 1;//chassis_power / wz_scale;
			}
      //speed limit
      //速度限幅
      chassis_move_control->vx_set = fp32_constrain(chassis_move_control->vx_set, chassis_move_control->vx_min_speed, chassis_move_control->vx_max_speed);
    ...
    else
    {
      ...
      //计算旋转PID角速度
      chassis_move_control->wz_set = -PID_calc(&chassis_move_control->chassis_angle_pid, chassis_move_control->chassis_yaw_motor->relative_angle, 0);
			//speed limit
      //速度限幅
      chassis_move_control->vx_set = fp32_constrain(chassis_move_control->vx_set, chassis_move_control->vx_min_speed, chassis_move_control->vx_max_speed);
      chassis_move_control->vy_set = fp32_constrain(chassis_move_control->vy_set, chassis_move_control->vy_min_speed, chassis_move_control->vy_max_speed); 
